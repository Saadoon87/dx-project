...
export default class UlsCalculator extends LightningElement {
  ...
  get isNextDisabled() {
    // Step 3, Step 4, and Step 5 are not implemented yet.
    // Keep the placeholder templates in the component, but block production navigation into them.
  ...
  get displayCustomerName() {
    // Apex should normally return customerName after backend-to-UI mapping.
    // lifeInsuredName is kept only as a defensive fallback for early integration testing.
    const customerName =
      this.policyData?.customerName !== undefined &&
      this.policyData?.customerName !== null
        ? this.policyData.customerName
        : this.policyData?.lifeInsuredName;

    return this.getSafeValue(customerName, "Not available");
  }
  ...
  get displayCurrentUav() {
    // Apex should normally return currentUav after backend-to-UI mapping.
    // currentUAV is kept only as a defensive fallback for early integration testing.
    const currentUav =
      this.policyData?.currentUav !== undefined &&
      this.policyData?.currentUav !== null
        ? this.policyData.currentUav
        : this.policyData?.currentUAV;

    return this.getSafeValue(currentUav, "Not available");
  }
  ...
  handleNext() {
    // Step 3, Step 4, and Step 5 are not implemented yet.
    // This guard prevents accidental navigation into unfinished placeholder screens.
    if (this.currentStep >= 2) {
      this.errorState = {
        userMessage:
          "The next step is not available yet. Please complete the current available steps.",
  ...
  setUnexpectedError() {
    // Do not expose raw JavaScript, Apex, or network exception details to the user.
    // Technical details should be investigated through Salesforce logs and browser dev tools by support/admins.
    this.errorState = {
      userMessage:
